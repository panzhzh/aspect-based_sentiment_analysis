...
# 读取CSV文件
df = pd.read_csv('./dataset/reviews_demo.csv')
...
def process_sentence(sentence):
    ...
    for topic, sentences in result_phrases.items():
        if sentences:
            ...
            reconstructed_sentence += f"{topic} is {combined_sentences}. "

    # 返回结果
    return noun_phrases, result_phrases, reconstructed_sentence.strip()

# 遍历每条评论
for review in df['review_text']:
    noun_phrases, result_phrases, reconstructed_sentence = process_sentence(review)
    results.append({
        'review_text': review,
        'noun_phrases': result_phrases,
        'reconstructed_sentence': reconstructed_sentence
    })

# 将结果写入新的CSV文件
output_df = pd.DataFrame(results)
output_df.to_csv('./dataset/output_reviews.csv', index=False)
