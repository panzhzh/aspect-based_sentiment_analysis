...
roberta_analyzer = pipeline('sentiment-analysis', model=roberta_model, tokenizer=roberta_tokenizer)
...
def expand_keywords(aspect_keywords, nlp, topn=10):
    ...
    for aspect, keywords in aspect_keywords.items():
        ...
        for keyword in keywords:
            token = nlp(keyword)[0]  # 获取单个Token对象
...
def extract_aspect_sentences(text, aspect_keywords):
    ...
    for sentence in sentences:
        ...
        # RoBERTa模型
        result = roberta_analyzer(sentence)[0]
