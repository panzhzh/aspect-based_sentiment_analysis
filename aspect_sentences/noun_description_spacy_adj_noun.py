...
sentence = "Good Filipino food! service was a winner. Service is good and staff are all friendly. Will come back again to try out other dishes on the menu."
...
# 定义一个函数来添加短语到noun_phrases
def add_phrase_to_noun_phrases(noun, phrase):
    if noun in noun_phrases:
        if phrase not in noun_phrases[noun]:  # 可选：避免添加重复的短语
            noun_phrases[noun].append(phrase)
    else:
        noun_phrases[noun] = [phrase]
...
for token in doc:
    if token.pos_ == 'NOUN':
        ...
        if processed_nouns[token.text] < noun_counts[token.text]:
            # 找到每个名词和它的修饰词
            modifiers = [child.text for child in token.children if child.dep_ in {'amod', 'det', 'nummod', 'compound'} and child.text != token.text]

            # 添加名词的修饰词组合到字典
            add_phrase_to_noun_phrases(token.text, ' '.join(modifiers).strip())
            ...
            if token_head.pos_ == 'ADJ':
                ...
                for sibling in token_head.children:
                    ...
                    elif sibling.pos_ == 'NOUN' and sibling.i > token_head.i:
                        # 如果是名词且出现在父节点之后，提取修饰词和名词短语
                        noun_modifiers = [child.text for child in sibling.children if
                                          child.dep_ in {'advmod', 'amod', 'det', 'neg', 'nummod', 'poss', 'prep',
                                                         'quantmod', 'acomp'}]
                        noun_phrase = ' '.join(noun_modifiers + [sibling.text]).strip()
                        if token.text in noun_phrases:
